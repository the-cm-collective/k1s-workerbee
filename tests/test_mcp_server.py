...
def test_project_runbook_update_mcp_tool_uses_daemon_update(
    tmp_path,
    monkeypatch,
) -> None:
    captured: dict[str, Any] = {}
    ...
    class FakeMCP:
        ...
        def run(self, *, transport: str) -> None:
            captured["transport"] = transport
            captured["result"] = self.tools["workerbee_v1_project_runbook_update"](
                content="Validated deploy path.",
                project="Alpha",
                mode="append",
                source="test",
                summary="deploy",
    ...
    class FakeDaemon:
        ...
        def project_runbook_update(self, **kwargs: Any) -> dict[str, Any]:
            captured["update"] = kwargs
            return {
                "project": kwargs["project"],
                "changed": True,
                "path": str(tmp_path / "project-runbook.md"),
            }
    ...
    assert result["kind"] == "ProjectRunbookUpdate"
    assert result["project"] == "Alpha"
...
def test_ingress_ca_regenerate_mcp_tool_requires_confirm_path(
    tmp_path,
    monkeypatch,
) -> None:
    captured: dict[str, Any] = {}
    ...
    class FakeMCP:
        ...
        def run(self, *, transport: str) -> None:
            captured["transport"] = transport
            captured["result"] = self.tools["workerbee_v1_ingress_ca_regenerate"](
                confirm=True
            )
            raise StopServe
    ...
    result = captured["result"]
    assert captured["transport"] == "streamable-http"
    assert result["kind"] == "IngressCARegenerate"
    assert result["data"]["regenerated"] is True
