...
def test_stop_mcp_daemon_keeps_metadata_when_stale_containerd_cleanup_fails(
    tmp_path: Path,
    monkeypatch,
) -> None:
    config = MCPDaemonConfig(
    ...
    monkeypatch.setattr(
        "workerbee.mcp_daemon._stop_containerd_state_before_helper_stop",
        lambda *_args: {"ok": False, "errors": [{"error": "cleanup failed"}]},
    )
    monkeypatch.setattr(
        "workerbee.mcp_daemon.stop_containerd_helper",
    ...
    assert result["containerd_helper_stop"]["stopped"] is False
