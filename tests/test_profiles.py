...
def test_profile_stop_removes_generated_ingress_route(tmp_path: Path) -> None:
    sites_dir = tmp_path / "global" / "caddy-sites" / "demo"
    route = sites_dir / "k1s-profile.caddy"
    route.parent.mkdir(parents=True, exist_ok=True)
    route.write_text(
        "# Generated by WorkerBee k1s profile runner.\n"
        "https://k1s.demo.workerbee.localhost { reverse_proxy 127.0.0.1:19608 }\n",
        encoding="utf-8",
    )
    runner = K1sProfileRunner(
        project="demo",
    ...
    assert result["ok"] is True
    assert result["ingress_route"]["removed"] is True
