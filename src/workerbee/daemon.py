        ...
        *,
        ...
        try:
            ...
            result["ingress_refresh"] = _profile_ingress_refresh_metadata(
                ...
                after_site_text=after_site_text,
                site_path=site_path,
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:  # noqa: N802
                ...
                _send_html(
                    self,
                    _render_dashboard(
                        _dashboard_shell_payload(daemon),
...
def _dashboard_shell_payload(daemon: WorkerBeeDaemon) -> dict[str, Any]:
    global_dashboard = daemon.global_dashboard()
    projects: list[dict[str, Any]] = []
    return {
        "state_root": str(daemon.state_root),
        "global_dashboard": global_dashboard,
        "projects": projects,
        "summary": _dashboard_summary(projects, global_dashboard=global_dashboard),
        "action_jobs": daemon.dashboard_action_jobs(),
        "ingress_sync": {"needed": False, "synced": False},
        "updated_at": time.time(),
    }
...
def _remove_generated_stack_ingress_site(sites_dir: Path) -> bool:
    return _remove_generated_caddy_site(
        sites_dir / "k1s-stack.caddy",
        "# Generated by WorkerBee stack supervisor.",
    )


def _remove_generated_profile_ingress_site(
    site_path: Path | None,
    text: str | None = None,
) -> bool:
    return _remove_generated_caddy_site(
...
def _remove_generated_caddy_site(
    path: Path | None,
    header: str,
    *,
    text: str | None = None,
    ...
    if path is None or not path.is_file():
        return False
    site_text = text if text is not None else _read_optional_text(path)
    if not site_text or not site_text.startswith(header):
        return False
    try:
        path.unlink()
        return True
...
def _profile_ingress_refresh_metadata(
    *,
    ...
    if error:
        ...
        reason = "stopped profile ingress route removed"
