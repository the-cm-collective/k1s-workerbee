...
INGRESS_RELOAD_TIMEOUT_SECONDS = 30
INGRESS_HEALTH_VERIFY_TIMEOUT_SECONDS = 10.0
    ...
    def reload(self) -> dict[str, Any]:
        ...
        try:
            proc = subprocess.run(
                ...
                timeout=INGRESS_RELOAD_TIMEOUT_SECONDS,
            )
            ...
            return {
                ...
                "returncode": None,
                "reason": "caddy reload timed out",
                "timeout_seconds": INGRESS_RELOAD_TIMEOUT_SECONDS,
...
"""
        if self.dashboard_port:
            dashboard_handlers = f"""
    @workerbee_health path /healthz
    handle @workerbee_health {{
        respond "ok\\n" 200
    }}
    reverse_proxy {self.host_alias}:{self.dashboard_port}
"""
    ...
    respond "WorkerBee dashboard backend is not enabled in this runtime\\n" 503
"""
...
""",
    ...
    def _verified_dashboard_health_probe(self) -> dict[str, Any]:
        deadline = time.monotonic() + INGRESS_HEALTH_VERIFY_TIMEOUT_SECONDS
        last_probe: dict[str, Any] = {}
        while True:
            last_probe = _global_dashboard_health_probe(self.info().public_dict())
            if last_probe.get("ok"):
                return last_probe
            if time.monotonic() >= deadline:
                last_probe["verification_timeout_seconds"] = (
                    INGRESS_HEALTH_VERIFY_TIMEOUT_SECONDS
                )
                return last_probe
            time.sleep(0.2)
