...
class K1sProfileRunner:
    ...
    def _remove_ingress_site(self) -> dict[str, Any]:
        if not self.ingress:
            return {"removed": False, "enabled": False}
        site = self.ingress.sites_dir / "k1s-profile.caddy"
        if not site.is_file():
            return {"removed": False, "enabled": True, "path": str(site)}
        try:
            text = site.read_text(encoding="utf-8")
        except OSError as exc:
            return {"removed": False, "enabled": True, "path": str(site), "error": str(exc)}
        if not text.startswith("# Generated by WorkerBee k1s profile runner."):
            return {"removed": False, "enabled": True, "path": str(site), "reason": "not generated"}
        try:
            site.unlink()
        except OSError as exc:
            return {"removed": False, "enabled": True, "path": str(site), "error": str(exc)}
        return {"removed": True, "enabled": True, "path": str(site)}
