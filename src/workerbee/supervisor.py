...
class WorkerBeeSupervisor:
    ...
    def start(self) -> StackInfo:
        ...
        runtime_info = self.k1s_runtime
        docs_site_source = _resolve_docs_site_source(
            k1s_root=runtime_info.k1s_root,
            ae_origin=runtime_info.ae_origin,
            cwd=self.cwd,
        )
        ingress_urls = self._write_stack_ingress_sites(
            controller_port=controller_port,
            apishim_port=apishim_port,
            docs_site_source=docs_site_source,
        )
        ...
        info = StackInfo(
            ...
            k1s_root=str(runtime_info.k1s_root) if runtime_info.k1s_root else None,
            k1s_runtime_source=runtime_info.source,
    ...
    def _refresh_stack_ingress_info(self, info: StackInfo) -> StackInfo:
        ...
        docs_site_source = _resolve_docs_site_source(
            k1s_root=Path(info.k1s_root) if info.k1s_root else None,
            ae_origin=info.ae_origin,
            cwd=self.cwd,
        )
        ingress_urls = self._write_stack_ingress_sites(
            controller_port=info.controller_port,
    ...
    def _write_stack_ingress_sites(
        ...
        *,
        controller_port: int,
        apishim_port: int,
        docs_site_source: Path | None = None,
    ) -> dict[str, str]:
        if not self.ingress:
            return {}
        docs_site_root = self._sync_stack_docs_site(docs_site_source)
        urls = self._stack_ingress_urls(
            controller_port=controller_port,
            apishim_port=apishim_port,
            docs_site_available=docs_site_root is not None,
        )
        controller_host = self.ingress.host("k1s")
        legacy_dash_host = self.ingress.host("k1s-dash")
        api_host = self.ingress.host("k1s-api")
        docs_host = self.ingress.host("k1s-docs")
...
"""
        docs_site = ""
        if docs_site_root is not None:
            docs_site = f"""
https://{docs_host} {{
    header -Strict-Transport-Security
    tls internal
    root * /etc/caddy/projects/{self.project}/docs-site
    file_server
}}
"""
...
"""
    ...
    def _stack_ingress_urls(
        ...
        *,
        controller_port: int,
        apishim_port: int,
        docs_site_available: bool = False,
    ) -> dict[str, str]:
        ...
        if docs_site_available:
            urls["docs_site"] = self.ingress.url(self.ingress.host("k1s-docs"), "/")
        return urls

    def _sync_stack_docs_site(self, source: Path | None) -> Path | None:
        dest = self.state_dir / "docs-site"
        if source is not None and _looks_like_docs_site(source):
            if dest.exists():
                shutil.rmtree(dest)
            shutil.copytree(source, dest)
        if _looks_like_docs_site(dest):
            return dest
        return None
...
def _resolve_docs_site_source(
    *,
    ...
    roots: list[Path] = []
    if k1s_root is not None:
        roots.append(k1s_root)
    if ae_origin:
        origin_root = _k1s_root_from_ae_origin(Path(ae_origin))
        if origin_root is not None:
            roots.append(origin_root)
    roots.extend([cwd, cwd.parent / "k1s"])
    seen: set[Path] = set()
    for root in roots:
        ...
        if resolved in seen:
            continue
        seen.add(resolved)
        docs_site = resolved / "docs" / "site"
        if _looks_like_docs_site(docs_site):
            return docs_site
    return None


def _k1s_root_from_ae_origin(path: Path) -> Path | None:
    parts = path.parts
    if len(parts) >= 4 and parts[-3:] == ("src", "ae", "__init__.py"):
        return Path(*parts[:-3])
    return None


def _looks_like_docs_site(path: Path) -> bool:
    return path.is_dir() and (
        (path / "index.html").is_file() or (path / "start-here.html").is_file()
    )
