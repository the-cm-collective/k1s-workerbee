...
class RemoteK1sTarget:
    ...
    def _write_ingress_sites(self, *, controller_port: int, apishim_port: int) -> dict[str, str]:
        if not self.ingress:
            return {}
        controller_host = f"k1s-remote.{self.project}.workerbee.localhost"
        api_host = f"k1s-remote-api.{self.project}.workerbee.localhost"
    ...
    def _api_public_base(self, apishim_port: int) -> str:
        if self.ingress:
            return self.ingress.url(
                f"k1s-remote-api.{self.project}.workerbee.localhost",
                "/",
            ).rstrip("/")
