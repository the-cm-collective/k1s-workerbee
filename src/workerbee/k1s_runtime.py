...
def resolve_k1s_runtime(
    *,
    ...
    installed = _probe_ae(python_bin, pythonpath=None)
...
def _runtime_from_root(root: Path) -> K1sRuntime:
    ...
    origin = _probe_ae(python_bin, pythonpath=pythonpath)
    if origin is None:
...
print(json.dumps({"origin": getattr(ae, "__file__", None)}))
"""
    proc = subprocess.run(
        [python_bin, "-c", code],
        env=env,
    ...
    if proc.returncode != 0:
        return None
    try:
        return str(json.loads(proc.stdout)["origin"])
    except Exception:
        return None
